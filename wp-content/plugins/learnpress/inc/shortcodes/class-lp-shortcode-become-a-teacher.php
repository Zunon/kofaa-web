...
if ( ! class_exists( 'LP_Shortcode_Become_A_Teacher' ) ) {
	...
	class LP_Shortcode_Become_A_Teacher extends LP_Abstract_Shortcode {
		...
		public function __construct( $atts = '' ) {
			...
			self::add_message( sprintf( __( 'رجاء %s لإرسال طلبك!', 'learnpress' ), sprintf( '<a href="%s">%s</a>',learn_press_get_login_url(), _x( 'login', 'become-teacher-form', 'learnpress' ) ) ), 'login' );
			...
			self::add_message( sprintf( __( 'Please %s to send your request!', 'learnpress' ), sprintf( '<a href="%s">%s</a>',learn_press_get_login_url(), _x( 'login', 'become-teacher-form', 'learnpress' ) ) ), 'login' );
				...
				if ( self::has_sent() ) {
					if(get_bloginfo("language") == "ar") { 
			self::add_message( __( 'لقد قمت بالفعل بإرسال الطلب. الرجاء انتظار الموافقة.', 'learnpress' ), 'sent' );
			...
			if(get_bloginfo("language") == "ar") { 
			self::add_message( __( 'انت مدرس!', 'learnpress' ), 'is-teacher' );
