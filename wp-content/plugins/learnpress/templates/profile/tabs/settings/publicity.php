...
<form method="post" name="profile-publicity" enctype="multipart/form-data" class="learn-press-form">
    ...
    <ul class="form-fields">
        ...
        <li class="form-field">
            <label for="my-dashboard"><?php 
			
if(get_bloginfo("language") == "ar") { 
					_e( 'لوحة القيادة الخاصة بي', 'learnpress' );
			}else {
				_e( 'My dashboard', 'learnpress' );
			} ?></label>
            <div class="form-field-input">
                ...
                <p class="description">
                    <?php
					if(get_bloginfo("language") == "ar") { 
				_e( 'محتوى ملف تعريف المستخدم العام ، إذا تم إيقاف تشغيل هذا الخيار ، فستصبح الأقسام الأخرى في الملف الشخصي غير مرئية أيضًا
.', 'learnpress' );
			}else {
			_e( 'Public user profile content, if this option is turn off then other sections in profile also become invisible.', 'learnpress' );
			}
		...
		<?php if ( LP()->settings()->get( 'profile_publicity.courses' ) === 'yes' ) { ?>
            <li class="form-field">
				...
				<?php 
			if(get_bloginfo("language") == "ar") { 
				learn_press_display_message( __( 'دوراتي', 'learnpress-certificates' ) );
			}else {
				_e( 'My Courses', 'learnpress' );
			}
		...
		<?php if ( LP()->settings()->get( 'profile_publicity.quizzes' ) === 'yes' ) { ?>
            <li class="form-field">
                <label for="my-quizzes"><?php if(get_bloginfo("language") == "ar") { 
    ...
    <button type="submit" name="submit" id="submit"><?php if(get_bloginfo("language") == "ar") { 
			_e( 'احفظ التغييرات', 'learnpress' );
				
			}else {
			_e( 'Save changes', 'learnpress' );
			}	 ?></button>
