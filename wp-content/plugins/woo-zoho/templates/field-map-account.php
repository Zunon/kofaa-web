...
.vxcf_options_row .howto{
  ...
  <div id="crm_fields_map" class="vx_contents">
    <div class="vx_div ">
  <div class="vx_head ">
<div class="crm_head_div"> <?php _e('1. Select Zoho Account',  'woo-zoho' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vxc_account"><?php _e('Select Zoho Account.', 'woo-zoho');  $this->tooltip($tooltips['sel_object']); ?></label>
  </div>
  <div class="vx_col2">
  <div>
  ...
  echo '<option value="'.$f_val['id'].'" '.$select.'>'.$f_val['name'].'</option>';    
  }    
  ?>
  </select>
  </div>
  ...
  <div id="crm_ajax_account" style="display:none; text-align: center; line-height: 100px;"><i><?php _e('Loading, Please Wait...', 'woo-zoho'); ?></i></div>
    <?php 
    $valid=true;
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e('This is a required field','woo-zoho') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  button_state_vx("ajax",btn);
   var arr={action:'refresh_data_<?php echo $this->id ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account,post_id:post_id};
 ...
 if(re.data){
  ...
  button_state_vx("ajax",btn);
   var account=$("#vxc_account").val();
  $.post(ajaxurl,{action:'get_objects_<?php echo $this->id ?>',account:account,vx_crm_ajax:vx_crm_ajax},function(res){
  var re=$.parseJSON(res);
  button_state_vx("ok",btn);  
  if(re.html){
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php _e('Please Select Field Name','woo-zoho') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  e.preventDefault(); 
  if(!confirm('<?php _e('Are you sure to remove ?','woo-zoho') ?>')){
      return;
  }
  ...
  ajax.show();
  fields_div.hide();
  vx_ajax=$.post(ajaxurl,{action:'field_account_<?php echo $this->id ?>',account:account,id:'<?php echo $post_id ?>',vx_crm_ajax:vx_crm_ajax},function(res){
  fields_div.html(res);
...
start_tooltip();
  ...
  if(object == ""){
  alert("<?php _e('Please Select Object','woo-zoho') ?>");
  return;
  }
  if(vx_ajax && vx_ajax.abort){
    vx_ajax.abort();  
  ...
  var btn=$("#vx_refresh_fields");
    button_state_vx("ajax",btn);
  vx_ajax=$.post(ajaxurl,{action:'fields_map_<?php echo $this->id ?>',account:account,object:object,id:'<?php echo $post_id ?>',vx_crm_ajax:vx_crm_ajax},function(res){
  fields_div.html(res);
  fields_div.slideDown();    
...
$('#vx_add_fields_select').select2({ placeholder: '<?php _e('Select Field','woo-zoho') ?>'});
add_note_sel();
} 
  });
function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php _e('Select Field','woo-zoho') ?>'});
}  
