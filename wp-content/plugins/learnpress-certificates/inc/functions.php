		...
		foreach ( $socials as $k => $v ) {
			...
			switch ( $k ) {
				...
				case 'facebook':
					...
					<script
						type="text/javascript">LP_Certificate.loadJs('//connect.facebook.net/en_US/sdk.js#xfbml=1&version=v2.5')</script>
					<?php
					break;
				case 'plusone':
					?>
					...
					<script
						type="text/javascript">LP_Certificate.loadJs('https://apis.google.com/js/plusone.js')</script>
					<?php
					break;
			}
...
function learn_press_certificate_generate_image_certificate( $cert, $context = 'save' ) {
	...
	if ( $context == 'save' ) {
		...
		$img_thumb_url = str_replace( '\\', '/', $img_thumb_url );
