...
<div id="learn-press-profile-nav">
    ...
    <ul class="learn-press-tabs tabs">
		...
		foreach ( $profile->get_tabs()->tabs() as $tab_key => $tab_data ) {
			...
			$link = get_site_url()."/profile/".$user_login."/".$tab_key."?lang=".get_bloginfo("language");
			if(get_bloginfo("language") != "ar") { 
				$link = get_site_url().'/'.apply_filters( 'wpml_current_language', NULL )."/profile/".$user_login."/".$tab_key;
			}
				...
				<?php if ( $sections && sizeof( $sections ) > 1 ) { ?>
                    ...
                    <ul class="profile-tab-sections">
						<?php foreach ( $sections as $section_key => $section_data ) {
							...
							$section_link = get_site_url()."/profile/".$user_login."/".$tab_key."/".$section_slug."?lang=".get_bloginfo("language");
							if(get_bloginfo("language") != "ar") { 
							$section_link = get_site_url().'/'.apply_filters( 'wpml_current_language', NULL )."/profile/".$user_login."/".$tab_key."/".$section_slug;
		...
		<?php } ?>
		<li class="calendar"><a href="<?php echo get_site_url().'/event?lang='.get_bloginfo("language");?>"> <?php if(get_bloginfo("language") == "ar") { echo "عرض التقويم"; }else { echo "Calendar"; } ?></a></li>
