...
if ( ! function_exists( 'learn_press_become_teacher_heading' ) ) {
	...
	function learn_press_become_teacher_heading() {
		...
		if(get_bloginfo("language") == "ar") { 
			_e( 'املأ النموذج وأرسل إلينا طلبك', 'learnpress' );
				
			}else {
			_e( 'Fill out the form and send us your request', 'learnpress' );
			}	
...
if ( ! function_exists( 'learn_press_back_to_class_button' ) ) {
	function learn_press_back_to_class_button() {
		...
		<a href="<?php echo learn_press_get_page_link( 'courses' ); ?> " class="green_color font18px bck_to_cours" ><?php _e( 'العودة إلى الدورات', 'learnpress' ); ?></a>
	   </div>
		<?php
	}
}
