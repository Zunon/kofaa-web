...
public function refunded_order($id){ 
  ...
  */
  ...
  if($contents!=""){
  echo "<div class='error'><p>".$contents."</p></div>"; 
  ...
  */
  ...
  echo '<li><a href="' . admin_url( 'admin.php?page=wc-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section == $id ? 'current' : '' ) . '" title="'.$label.'">' . $label . '</a> ' . ( end( $array_keys ) == $id ? '' : '|' ) . ' </li>';
  }
...
$sel_val=array($sel_val);
 ...
 $val=ucfirst($arr_key)." ";
  ...
  $options.="<optgroup label='".$arr_val['title']."'>";
 if(isset($arr_val['fields']) && is_array($arr_val['fields'])){
  ...
  $options.='<option value="'.$f_key.'" '.$select.'>'.$val.$f_val['label'].'</option>';    
  } 
  } }
  $options.="</optgroup>"; 
  }
...
if(!empty($v['placeholder'])){ $label=$v['placeholder']; }
 ...
 $user=array('__vx_wp-user_login'=>array('label'=>'user_login'),'__vx_wp-ID'=>array('label'=>'user_id')
  ...
  $tx_arr=array();
  $product_attrs=array('name'=>'Product Name','description'=>'Product Description','short_description'=>'Short Description','sku'=>'Product SKU','price'=>'Product Price','regular_price'=>'Product Regular Price','sale_price'=>'Product Sales Price','total_sales'=>'Product Total Sales','stock_quantity'=>'Stock Quantity','weight'=>'Product Weight','length'=>'Product Length','get_category_ids'=>'Product Categories','get_category'=>'Product First Category','product_img'=>'Product Image URL');
...
$item_fields=array('__vxp_iun-name'=>array('label'=>'Item Name'),'__vxp_iun-type'=>array('label'=>'Item Type'),'__vxp_iun-id'=>array('label'=>'Item ID'));
  ...
  */
  ...
  ?>
  <div class="alert_danger"><?php _e('Please Select Object','woo-zoho')?></div>
  <?php
  return;
  }
...
$data=$info['data'];
  ...
  $fields=$api->get_crm_fields($module); 
     ...
     $temp_fields=array(); $phones=array('Phone','AsstPhone','OtherPhone','HomePhone','Home_Phone','Other_Phone','Asst_Phone','tags','vx_attachments','GCLID','mobile','vx_ship_entry');
    foreach($fields as $k=>$v){
...
$meta= is_array($meta) ? $meta : array(); 
 ...
 $fields=$this->post('fields',$feed); 
  ...
  $fields=__("No Fields Found",'woo-zoho');    
   ...
   ?>
  <div class="vx_error"><?php echo $fields ?></div>
  <?php
  return;
  }
...
if($object_match && $account_match && $feed_match ){    
  ...
  */
  ...
  $html="<option value=''>".__("Select Object",'woo-zoho')."</option>";
  $res=array();
...
if($check && !empty($info)){
  ...
  */
  ...
  ?>
  <div class="alert_danger"><?php _e('Please Select a Zoho Account','woo-zoho')?></div>
  <?php
  die();
  }
  $this->post_id=$id=$this->post('id');
