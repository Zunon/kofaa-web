...
<!-- echo esc_url( learn_press_get_checkout_url() ); -->
    <form method="post" id="learn-press-checkouts" name="learn-press-checkout" class="learn-press-checkout checkout<?php echo ! is_user_logged_in() ? " guest-checkout" : ""; ?>" action="<?php echo plugin_dir_url(__FILE__)."payment/paytabs.php"; ?>" enctype="multipart/form-data">
...
<?php if ( ! is_user_logged_in() && ! LP()->checkout()->is_enable_login() && ! LP()->checkout()->is_enable_register() ) { ?>
    <p><?php printf( __( 'يرجى تسجيل الدخول لمتابعة الخروج. %s', 'learnpress' ), sprintf( '<a href="%s" data-toggle="modal" class="green_color" data-target="#loginModal" >%s</a>', learn_press_get_login_url(), __( 'Login', 'learnpress' ) ) ); ?></p>
<?php } ?>
