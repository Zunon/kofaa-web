...
if($_POST) {
	...
	if($_GET["t"] == "cancel")
	{
		...
		echo "<a href='".home_url("/courses")."'> click here for re purchange </a>";
	}
	else
	{
		$current_user = get_user_by_email($_GET["u_email"]);
			...
			$transact_no = $_POST['transaction_id'];
			$total_value = $_POST['transaction_amount'];
			$course_id = $_GET["prod_id"];
			...
			$wpdb->insert("wp_posts", array(
				...
				"post_status" => ($payment_status == '100' ? 'lp-completed' : 'lp-pending'),
			...
			$wpdb->insert("wp_postmeta", array(
				...
				"meta_value" => 'Paytab Debit/Credit Card'
			));
			$wpdb->insert("wp_postmeta", array(
				"post_id" => $post_id,
			...
			$wpdb->insert("wp_paytab_custom_payments", array(
				"order_id" => $order_id,
				"course_id" => $course_id,
				"user_id" => $user_id,
			...
			$wpdb->insert("wp_learnpress_user_items", array(
				...
				"item_id" => $course_id,
				"item_type" => 'lp_course',
				"status" => ($payment_status == '100' ? 'enrolled' : 'pending'),
				"ref_id" => $order_id,
				"ref_type" => 'lp_order'
			));
			...
			$wpdb->insert("wp_learnpress_user_itemmeta", array(
				...
				"meta_value" => ($payment_status == '100' ? 'purchased' : '')
			));
			$wpdb->insert("wp_learnpress_user_itemmeta", array(
				"learnpress_user_item_id" => $learnpress_user_item_id,
				"meta_key" => '_current_status',
				"meta_value" => ($payment_status == '100' ? 'enrolled' : 'pending')
			));
			$wpdb->insert("wp_learnpress_user_itemmeta", array(
				"learnpress_user_item_id" => $learnpress_user_item_id,
								 ...
								 <table style="width:100%;margin-bottom:20px">
									<tbody>
									   ...
									   <tr>
										  ...
										  <td style="text-align:right;padding:5px 0">Paytab Debit/Credit Card</td>
									   </tr>
									   <tr>
			...
			if($_GET["u_email"] == "ar")
			{
				$thankupage = home_url('/شكرا-لك-على-الشراء/?lang=ar');
			}
			else
			{
				$thankupage = home_url('/thank-you-for-purchase');
			}
			
		    header("Location:". $thankupage);
	}
