...
add_action( 'profile_update',array($this,'profile_update'), 40 );
...
public function save_product($post_id,$product){
   ...
   if( !defined( 'DOING_CRON' ) && method_exists($product,'get_status') && $product->get_status() == 'publish' ){  
...
if(!$is_subscription){
  ...
  $date=self::$_order->get_date_created()->format('d-M-Y H:i:s');
  } 
